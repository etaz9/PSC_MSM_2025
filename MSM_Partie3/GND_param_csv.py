...
betas = [0.5, 1, 1.5, 2, 3, 8]

# Création des en-têtes
ws.cell(row=1, column=1).value = "kbar"
for idx, beta in enumerate(betas):
    ws.cell(row=1, column=idx+2).value = f"beta={beta}"
...
for kbar in range(1, 8):  # kbar marque les lignes
    ...
    for idx, beta in enumerate(betas):  # df marque les colonnes
        # Définir l'index de la colonne dans le fichier Excel
        excel_col = idx + 2

        print(f"Traitement de kbar={kbar}, beta={beta}")

        # Appel de ta fonction de calcul (à adapter avec tes propres variables et imports)
        parameters, LL, niter, output = glo_min(kbar, dat_real, niter, temperature, stepsize)

        # Stockage des meilleures valeurs
        if LL < best_LL:
            best_LL = LL
            best_kbar = kbar
            best_df = beta
        ...
        cell_text = (
            ...
            f"b= {parameters[0]}\n"
            f"m0= {parameters[1]}\n"
            f"gamma_kbar= {parameters[2]}\n"
            f"sigma= {parameters[3]}"
        )
...
for col in ws.columns:
    ...
    for cell in col:
        if cell.value:
            # Prendre la longueur de la première ligne du contenu pour ajuster
...
excel_filename = "Valeurs_log-likelihood_kbar_beta_2024.xlsx"
try:
    wb.save(excel_filename)
    print(f"Fichier Excel créé avec succès: {excel_filename}")
    print(f"Meilleures valeurs trouvées: kbar={best_kbar}, beta={best_df}, LL={best_LL}")
except PermissionError:
    # Si le fichier est ouvert ailleurs ou protégé
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_filename = f"Valeurs_log-likelihood_kbar_beta_2024_{timestamp}.xlsx"
    wb.save(excel_filename)
    print(f"Fichier Excel créé avec succès: {excel_filename}")

print(f"La meilleure log-likelihood est {best_LL},atteinte pour kbar={best_kbar}")
