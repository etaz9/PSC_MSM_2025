...
def g_LL(m0, kbar, data, theta_in):
    ...
    #changing the distribution of returns
    #w_t = stats.t.pdf(w_t / s, df=5) / s
...
# # "------------------------SIMULATION----------------------------------"
def compute_MSM_vol(data_r):
    """used to be called from outside to return both estimation and in-sample volatility"""
    niter = 1
    temperature = 1.0
    stepsize = 1.0
    kbar= 5
    parameters, LL, niter, output = glo_min(kbar, data_r, niter, temperature, stepsize)
    ...
    pi_mat = g_pi_t(m0,kbar,data_r,[b, gamma_kbar, sigma])

    def compute_state_variances(m0, k_bar):
        """
        Computes the variance values (sigma^2) for each of the 2**k_bar states in the MSM.
        """
        m1 = 2 - m0
        states = np.array([
            m0**(k_bar - np.sum(np.array(list(np.binary_repr(i, width=k_bar)), dtype=int))) *
            m1**(np.sum(np.array(list(np.binary_repr(i, width=k_bar)), dtype=int)))
            for i in range(2**k_bar)
        ])
        variances = (sigma**2) * states  # Variances directly (not standard deviations)
        return variances

    def msm_volatility(pi_t, m0, k_bar):
        """
        Computes the conditional volatility at time t in the MSM model.
        """
        state_variances = compute_state_variances(m0, k_bar)
        conditional_variance = np.dot(pi_t, state_variances)
        conditional_volatility = np.sqrt(conditional_variance)
        return conditional_volatility

    return([np.array([msm_volatility(pi_t, m0, kbar) for pi_t in pi_mat]),LL,kbar,parameters])
